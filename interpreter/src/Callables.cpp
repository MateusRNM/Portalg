...
PortalgValue NativeEscreva::call(Interpreter *interpreter, const std::vector<PortalgValue> &arguments) {
...
PortalgValue NativeEscreval::call(Interpreter *interpreter, const std::vector<PortalgValue> &arguments) {
    for (const auto &arg : arguments) {
        std::string text = interpreter->stringify(arg);
        #ifdef __EMSCRIPTEN__
            write_in_terminal(text.c_str());
...
PortalgValue NativeLeia::call(Interpreter *interpreter, const std::vector<PortalgValue> &arguments) {
    #ifdef __EMSCRIPTEN__
...
PortalgValue NativeRaiz::call(Interpreter *interpreter, const std::vector<PortalgValue>& arguments) {
    ...
    if(std::holds_alternative<double>(arguments[0])) {
        radicand = std::get<double>(arguments[0]);
    } else if(std::holds_alternative<long long>(arguments[0])) {
        radicand = (double)std::get<long long>(arguments[0]);
    } else {
    ...
    long long degree = 2;
    if(arguments.size() == 2) {
        if(!std::holds_alternative<long long>(arguments[1])) {
            throw std::runtime_error("O índice da função 'raiz' deve ser um número inteiro.");
        }
        degree = std::get<long long>(arguments[1]);
    }
...
PortalgValue NativeLog::call(Interpreter *interpreter, const std::vector<PortalgValue>& arguments) {
    double logaritm = 0.0;
    if(std::holds_alternative<double>(arguments[0])) {
        logaritm = std::get<double>(arguments[0]);
    } else if(std::holds_alternative<long long>(arguments[0])) {
        logaritm = (double)std::get<long long>(arguments[0]);
    } else {
    ...
    if(std::holds_alternative<double>(arguments[1])) {
        base = std::get<double>(arguments[1]);
    } else if(std::holds_alternative<long long>(arguments[1])) {
        base = (double)std::get<long long>(arguments[1]);
    } else {
...
PortalgValue NativeArredondaCima::call(Interpreter *interpreter, const std::vector<PortalgValue>& arguments) {
    double value = 0.0;
    if(std::holds_alternative<double>(arguments[0])) {
        value = std::get<double>(arguments[0]);
    } else if(std::holds_alternative<long long>(arguments[0])) {
        value = (double)std::get<long long>(arguments[0]);
    } else {
...
PortalgValue NativeArredondaBaixo::call(Interpreter *interpreter, const std::vector<PortalgValue>& arguments) {
    double value = 0.0;
    if(std::holds_alternative<double>(arguments[0])) {
        value = std::get<double>(arguments[0]);
    } else if(std::holds_alternative<long long>(arguments[0])) {
...
PortalgValue NativeAleatorio::call(Interpreter *interpreter, const std::vector<PortalgValue>& arguments) {
    double min = 0.0;
    if(std::holds_alternative<double>(arguments[0])) {
        min = std::get<double>(arguments[0]);
    } else if(std::holds_alternative<long long>(arguments[0])) {
        min = (double)std::get<long long>(arguments[0]);
    } else {
    ...
    double max = 0.0;
    if(std::holds_alternative<double>(arguments[1])) {
        max = std::get<double>(arguments[1]);
    } else if(std::holds_alternative<long long>(arguments[1])) {
        max = (double)std::get<long long>(arguments[1]);
    } else {
